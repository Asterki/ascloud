	...
	const register = async () => {
		...
		if (password !== repeatPassword)
			return setModalState({
				open: true,
				title: "Error",
				message: lang.modal.errors["password-match"],
			});
		...
		try {
			...
			// Send the request
			const response: AxiosResponse<AccountServiceAPITypes.RegisterResponse> = await axios({
				url: "/api/accounts/register",
				method: "POST",
				data: parsedBody.data as AccountServiceAPITypes.RegisterRequestBody,
			});
			...
			switch (response.data) {
				case "username-email-in-use":
					setModalState({
						open: true,
						title: "Error",
						message: lang.modal.errors["username-in-use"],
					});
					break;
				...
				case "server-error":
					setModalState({
						open: true,
						title: "Error",
						message: lang.modal.errors["unknown"],
					});
					break;
		...
		<div className={styles["page"]}>
			...
			<main>
				...
				>
					...
					<input
						type="password"
						placeholder={lang.form.repeatPasswordPlaceholder}
					...
					<br />
					<p>
						{lang.form.haveAccount.split("&")[0]}{" "}
						<Link href="/login">{lang.form.haveAccount.split("&")[1]}</Link>
					</p>
				</motion.div>
			</main>
