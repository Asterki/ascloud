...
// TFA Related
router.post("/activate-tfa", async (req: express.Request<unknown, RouteTypes.ActivateTFAResponse, RouteTypes.ActivateTFARequestBody>, res: express.Response<RouteTypes.ActivateTFAResponse>) => {
...
router.post("/deactivate-tfa", async (req: express.Request<unknown, RouteTypes.DeactivateTFAResponse, RouteTypes.DeactivateTFARequestBody>, res: express.Response<RouteTypes.DeactivateTFAResponse>) => {
    if (!req.isAuthenticated() || req.user == undefined) return res.status(403).send("unauthorized");
